...
class OrderRemoteDatasource {
  ...
  Future<Either<String, OrderBuyerResponseModel>> getOrderBuyerId() async {
    final token = await LocalDatasource().getToken();
    final id = await LocalDatasource().getId();
    final response = await http.get(
      Uri.parse(
          '${ApiServices.baseUrl}/api/orders?filters[buyerId][\$eq]=$id}'),
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
        'Authorization': 'Bearer $token',
      },
    ...
    if (response.statusCode == 200) {
      return Right(OrderBuyerResponseModel.fromJson(response.body));
    } else if (response.statusCode == 201) {
      return Right(OrderBuyerResponseModel.fromJson(response.body));
    } else if (response.statusCode == 400) {
      return Right(OrderBuyerResponseModel.fromJson(response.body));
